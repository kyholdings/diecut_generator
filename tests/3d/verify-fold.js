...
// 1. 盒体四壁 + 盖面
const Hw = 63, L = 200, y1 = 63, y2 = 213;
...
const p = panels.find(x => x.id === 'left_outer');
const hookL = wp('left_outer', [p.localRect[0], (p.localRect[1] + p.localRect[3]) / 2, 0]);
C(Math.abs(hookL[2]) < 8, '左外段钩端到达盒底 z≈0', hookL.map(v => +v.toFixed(1)).join(','));
const p2 = panels.find(x => x.id === 'right_outer');
const hookR = wp('right_outer', [p2.localRect[2], (p2.localRect[1] + p2.localRect[3]) / 2, 0]);
C(Math.abs(hookR[2]) < 8, '右外段钩端到达盒底 z≈0', hookR.map(v => +v.toFixed(1)).join(','));

// 3. 插舌下端到达盒内（z 接近盒底，且在前壁内 y=63）
...
C(Math.abs(tuckLow[1] - y1) < 2, '插舌在前壁内侧 y=63', tuckLow.map(v => +v.toFixed(1)).join(','));

// 4. 外段在盒内（x 方向不越界）
C(hookL[0] > -2 && hookL[0] < L + 2, '左外段在盒内 x', hookL.map(v => +v.toFixed(1)).join(','));
C(hookR[0] > -2 && hookR[0] < L + 2, '右外段在盒内 x', hookR.map(v => +v.toFixed(1)).join(','));
